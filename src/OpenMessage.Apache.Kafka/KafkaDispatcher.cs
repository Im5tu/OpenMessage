...
namespace OpenMessage.Apache.Kafka
{
    internal sealed class KafkaDispatcher<T> : KafkaClient, IDispatcher<T>
    {
        ...
        public KafkaDispatcher(ILogger<KafkaDispatcher<T>> logger,
            IOptionsMonitor<KafkaOptions<T>> options,
            ...
            : base(logger)
        {
            ...
            _producer = new ProducerBuilder<byte[], byte[]>(options.CurrentValue.KafkaConfiguration)
        ...
        private Headers CreateHeadersFromExisting(Message<T> message)
        {
            ...
            if (headers.Count == 0) return HeadersIncludingDefaults(headers);

            bool hasContentType = false, hasValueType = false;
